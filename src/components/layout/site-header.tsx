...
export function SiteHeader({ className }: { className?: string }) {
  ...
  return () => unsub(); }, []); useEffect(() => { function onOpen(e: Event) { const detail = (e as CustomEvent).detail as { tab?: ActivityTab } | undefined; setPanelTab(detail?.tab || "cart"); setPanelOpen(true); setMenuOpen(false); } window.addEventListener("pm:open-panel", onOpen as EventListener); return () => window.removeEventListener("pm:open-panel", onOpen as EventListener); }, []); useEffect(() => { document.body.style.overflow = menuOpen || panelOpen ? "hidden" : ""; return () => { document.body.style.overflow = ""; }; }, [menuOpen, panelOpen]); function openPanel(tab: string) { setPanelTab(tab as ActivityTab); setPanelOpen(true); setMenuOpen(false); } function openMega(type: "categories" | "brands" | "sale") { if (megaTimer.current) clearTimeout(megaTimer.current); setMega(type); } function closeMegaDelayed() { if (megaTimer.current) clearTimeout(megaTimer.current); megaTimer.current = setTimeout(() => setMega(null), 150); } function submitTrack(e: React.FormEvent) { e.preventDefault(); const code = trackCode.trim(); if (!code) return; window.location.href = `/dashboard?track=${encodeURIComponent(code)}`; setMenuOpen(false); } 
  const pathname = usePathname();
  const [hideForHeroIntro, setHideForHeroIntro] = useState(false);

  useEffect(() => {
    if (pathname !== "/") {
      setHideForHeroIntro(false);
      return;
    }
    let done = false;
    try {
