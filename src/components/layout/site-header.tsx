...
<button type="button" className={item} title="سبد خرید" onClick={() => onOpenPanel?.("cart")}> <ShoppingCart className="h-4 w-4" /> <CountBadge count={counts.cart} /> </button> </div> );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/nav/mega");
        const data = await res.json();
        if (cancelled) return;
        if (type === "categories") {
          setItems(
            (data.categories ?? []).map((c: { name: string; href: string }) => ({
              name: c.name,
              href: c.href,
            })),
          );
          ...
          setItems(
            (data.brands ?? []).map((b: { name: string; href: string }) => ({
              name: b.name,
              href: b.href,
            })),
          );
  ...
  if (loading) {
    return (
      <div className="text-muted-foreground py-8 text-center text-sm">در حال بارگذاری…</div>
    );
  }
